...
def usaid_bf_fxn(date_after_var, country_name,username,api_key,creds_json,sheet_name):
    ...
    df = df.loc[df["Operating Unit"] == country_name]
    ...
    scatterplot = px.scatter(
        title = "USAID Business Forecast for " + country_name,
        data_frame=df,
        x = "Anticipated Solicitation Release Date",
        y = "Estimated Cost Minimum",
        color = "Sector",
        hover_name = "Award Title",
        hover_data=["Total Estimated Cost/Amount Range","Anticipated Award Date","Award Length","Award Description"],
        opacity = 0.9,
        orientation="v",
        template='gridon',
        labels={
    ...
    # py.plot(scatterplot, filename = 'USAID BF for' + country_name, auto_open = False)
...
for i in fh_dict_usaid_bf:
    usaid_bf_fxn(date_after_var,i,username,api_key,creds_json,sheet_name)
